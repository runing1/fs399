<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:49:"./application/admin/view/withdraw\withdrawal.html";i:1566467417;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
                    ...
                    <!--银行卡 模块  d-->
                         <!--可提现金额s-->
                                <div class="withdraw-Amounts">
                                    <p class="Amounts-p">可提现金额：<em>￥<?php if($user['user_money'] != 0): ?><?php echo $user['user_money']; else: ?>0.00<?php endif; ?></em></p>
                                    <div class="withdraw-Amounts-input">
                                        <i>￥</i>
                                        <ul>
                                            <li class="Amounts-li-tx p">
                                                <input class="Amounts-input fl" type="text" placeholder="<?php echo $user['user_money']; ?>" name="money"
                                                ...
                                                <input type="hidden" name="user_money" value="<?php echo $user['user_money']; ?>" id="user_money">
                                            </li>
...
<script type="text/javascript">
    ...
    $(".withdraw-select-dl dd").click(function () {
        var j = $(this).index();
        // var realname = '<?php echo $user['ali_username']; ?>';
        var realname = $('.Bindings-dev').html();
        //var bankusername='<?php echo $user['bank_username']; ?>';
        var bankusername = $('.Bindings-dev-bankusername').html();
        // var cash_alipay = '<?php echo $user['ali_account']; ?>';
        var cash_alipay = $('.Bindings-hone').html();
//    var cash_bank_card = '<?php echo $user['bank_card']; ?>';
...
<script type="text/javascript">
    ...
    $(document).on("click", '#add_card', function (e) {
        ...
        if (!card) {
            layer.open({content: '账号不能为空', icon: 2});
            return false;
        }
        if (!user_name) {
            layer.open({content: '真实姓名不能为空', icon: 2});
            return false;
        }
        if (isPone(card) || isMail(card)) {
            ...
            layer.open({content: '账号必须为手机号或邮箱', icon: 2});
            return false;
        }
    ...
    $(document).on("click", '#add_bank_card', function (e) {
        ...
        if (!card) {
            layer.open({content: '银行卡号不能为空', icon: 2});
            return false;
        }
        if (!cash_name) {
            layer.open({content: '开户名不能为空', icon: 2});
            return false;
        }
        if (isBankcard(card)) {
            ...
            layer.open({content: '请输入正确的银行卡号', icon: 2});
            return false;
        }
    ...
    //$('#update_withdrawalpwd').click(function(){
    //    //window.location.href="<?php echo U('user/withdrawal_pwd'); ?>";
    //    $('li[data-param="withdrawal_pwd|User"]').addClass('active');
    //});
    ...
    function checkSubmit() {
        ...
        var usermoney = parseFloat('<?php echo $user['user_money']; ?>');  //用户余额
...
//        }
        if (money > usermoney) {
            layer.open({content: '提现金额大于您的账户余额'});
            return false;
        }
        if (money < min_cash) {
            layer.open({content: '最小体现额度不能少于' + min_cash});
            return false;
        }
        if (paypwd == '') {
            layer.open({content: '请输入支付密码'});
            return false;
        }
        ajax_return_status = 0;
        $.ajax({
            type: "post",
            url: "<?php echo U('Admin/Withdraw/withdrawal'); ?>",
            dataType: 'json',
    ...
    function get_service() {
        if (cash_open == 1) {
            ...
            if (parseFloat(m) > parseFloat(max_cash)) {
                layer.open({content: '单次提现额不得大于' + max_cash, icon: 2, time: 3000});
                $('#money').val('');
                return false;
            }
            var r = get_taxfee(m);
    ...
    $('#all_cash').click(function () {
        $('#money').val('<?php echo $user['user_money']; ?>');

...
class Admin extends Base
{
    ...
    public function ajaxindex()
    {
        ...
        if (empty($keywords)) { // 开发者测试账号,测试用
            $res = D('admin_operator')->where('admin_id', 'not in', '2,3')->select();
        } else {
            $res = DB::name('admin_operator')->where('user_name', 'like', '%' . $keywords . '%')->where('admin_id', 'not in', '2,3')->order('admin_id')->select();
        }
        ...
        // $a=$Page->listRows;
        $list = $adminModel->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function modify_pwd()
    {
        ...
        if ($admin_id) {
            $info = D('admin_operator')->where("admin_id", $admin_id)->find();
            $info['password'] = "";
        ...
        if (IS_POST) {
            //修改密码
            $enOldPwd = encrypt_a($oldPwd . $info['ec_salt']);
            $enNewPwd = encrypt_a($newPwd . $info['ec_salt']);
            $admin = M('admin_operator')->where('admin_id', $admin_id)->find();
            if (!$admin || $admin['password'] != $enOldPwd) {
                ...
                $row = M('admin_operator')->where('admin_id', $admin_id)->save(array('password' => $enNewPwd));
                if ($row) {
                    exit(json_encode(array('status' => 1, 'msg' => '修改成功')));
                } else {
                    exit(json_encode(array('status' => -1, 'msg' => '修改失败,密码不能与原来一致')));
                }
            }
        }
        return $this->fetch();
    ...
    public function login()
    {
        if (IS_POST) {
            ...
            //获取ec_salt
            $ec_salt = M('admin_operator')->where(array('user_name' => $username))->field('ec_salt')->find();
...
//            }
    ...
    {
        $list = D('admin_operator_role')->order('role_id desc')->select();
    ...
    public function role_info()
    {
        ...
        $detail = array();
        if ($role_id) {
            $detail = M('admin_operator_role')->where("role_id", $role_id)->find();
    ...
    public function roleSave()
    {
        ...
        if (empty($data['role_id'])) {
            $admin_role = Db::name('admin_operator_role')->where(['role_name' => $res['role_name']])->find();
            if ($admin_role) {
                $this->error("已存在相同的角色名称!");
            } else {
                $r = D('admin_operator_role')->add($res);
            }
        } else {
            $admin_role = Db::name('admin_operator_role')->where(['role_name' => $res['role_name'], 'role_id' => ['<>', $data['role_id']]])->find();
            if ($admin_role) {
                $this->error("已存在相同的角色名称!");
            } else {
                $r = D('admin_operator_role')->where('role_id', $data['role_id'])->save($res);
            }
        }
        if ($r) {
            adminLog('管理角色');
    ...
    public function roleDel()
    {
        ...
        $admin = Db::name('admin_operator')->where('role_id', $role_id)->find();
        if ($admin) {
            exit(json_encode("请先清空所属该角色的管理员"));
        } else {
            $d = Db::name('admin_operator_role')->where("role_id", $role_id)->delete();
            if ($d) {
                exit(json_encode(1));
            } else {
    ...
    {
        $p = I('p/d', 1);
        $logs = DB::name('admin_operatorlog')->alias('l')->join('__ADMIN__ a', 'a.admin_id =l.admin_id')->order('log_time DESC')->page($p . ',20')->select();
        $this->assign('list', $logs);
        $count = DB::name('admin_operatorlog')->count();

...
class Saas
{
    ...
    public function ssoAdmin($username, $password)
    {
        ...
        $admin = Db::name('admin_operator')->alias('a')->join('__ADMIN_ROLE__ ar', 'a.role_id=ar.role_id')->where($condition)->find();
        if (!$admin) {
            return;
        }
    ...
    private function checkPrivilege()
    {
       ...
       // dump($actList);die;
        $right = Db::name('system_menu')->whereIn("id", $actList)->cache(true)->getField('right', true);
        $roleRight = '';

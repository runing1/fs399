...
class UsersLogic extends Model
{
    ...
    public function login($username,$password)
    {
        ...
        } elseif (encrypt_a($password) != $user['password']) {
    ...
    public function password($user_id,$old_password,$new_password,$confirm_password,$is_update=true){
        ...
        //验证原密码
        if($is_update && ($user['password'] != '' && encrypt_a($old_password) != $user['password']))
            return array('status'=>-1,'msg'=>'原密码验证失败','result'=>'');
        $row = M('users')->where("user_id", $user_id)->save(array('password'=>encrypt_a($new_password)));
        if(!$row)
    ...
    public function paypwd($user_id,$new_password,$confirm_password){
        ...
        $row = M('users')->where("user_id",$user_id)->update(array('paypwd'=>encrypt_a($new_password)));
        if(!$row){
            return array('status'=>-1,'msg'=>'修改失败','result'=>'');
        }

    ...
    private function addNormalCart()
    {
        ...
        if ($userCartGoods) {
            ...
            if ($this->specGoodsPrice) {
                ...
                $cartAddData['market_price'] = $this->specGoodsPrice['market_price'];
    ...
    private function addFlashSaleCart()
    {
        ...
        if ($userCartGoods) {
            ...
            if ($this->specGoodsPrice) {
                ...
                $cartAddFlashSaleData['market_price'] = $this->specGoodsPrice['market_price'];
...
//            $carts = $Cart->where(['combination_group_id' => $cart['combination_group_id'], 'id' => ['neq', $cart['id']]])->select();
            ...
            foreach ($carts as $cart_item) {
		        if($goods_num > $cart_item->limit_num){
                    // 回滚事务
                    Db::rollback();
...
//                $arr= $Cart->where(['combination_group_id' => $cartVal['combination_group_id'], 'id' => ['neq', $cartVal['id']]])->select();
        ...
        if ($cartListArr) {
            $cartListArr = collection($cartListArr)->append(['cut_fee', 'total_fee', 'goods_fee'])->toArray();
            $cartPriceInfo = $this->getCartPriceInfo($cartListArr);
            $cartPriceInfo['cartList'] = $cartListArr;
...
//        }
        ...
        foreach ($combination_goods_list as $combination_goods_item) {
            ...
            if (!empty($combination_goods_item['spec_goods_price'])) {
                ...
                $cart_combination_goods_item['market_price'] = $combination_goods_item['spec_goods_price']['market_price'];
            }

...
function qrcode($url,$filename){
   ...
   // var_dump($url);die;
    ...
    $object->png($url, "public/upload/tjm/".$filename.".png", $errorCorrectionLevel, $matrixPointSize, 2);
}
...
function random($length = 6, $numeric = 0) {
    ...
    if(!function_exists('getWinStock')){
        function getWinStock($user_id){
            //$number = db::name('goods')->where(['user_id'=>$user_id,'is_bbm'=>2])->value('goods_number');
            $recommend = db::name('goods')->where(['user_id'=>$user_id,'is_recommend'=>1,'is_bbm'=>'0','is_delete'=>0])->count();
            $onlin = db::name('goods')->where(['user_id'=>$user_id,'goods_status'=>['in',['onlin','uncheck']],'is_bbm'=>'0','is_delete'=>0])->count();

            return $recommend+$onlin;
        }
    }
    ...
    if (!function_exists('get_store_id')) {
        function get_store_id($userId){
            $id = Db::name('store')->where(['user_id'=>$userId])->value('id');
            return $id;
        }
    }


    require  'vendor/aliyun-oss-php-sdk/autoload.php';
    if(!function_exists('oss_upload_file')){
        /**
         *
         * 阿里云上传方法
         */
        function oss_upload_file($imageName = null,$water = false,$is_array = false)
        {
    ...
    if(!function_exists('locationSetInc')) {

        function locationSetInc($num)
        {
            //$num 父级推荐1星会员数
